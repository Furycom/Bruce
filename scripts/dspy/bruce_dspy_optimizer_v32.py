#!/usr/bin/env python3
"""
BRUCE DSPy Optimizer v3.2 (reprise + timeout)
=========================
...
log = logging.getLogger("dspy_v31")
...
STEP_TIMEOUT = 900  # 15 minutes max par step


def _load_partial_scores(label):
    """Charge scores des steps deja faits pour reprise."""
    sf = f"{RESULTS_DIR}/{label.lower()}_partial_scores.json"
    if os.path.exists(sf):
        try:
            with open(sf) as f:
                d = json.load(f)
            log.info(f"  [{label}] Reprise: {len(d['scores'])} steps deja faits, score={d.get('avg',0):.3f}")
            return d["scores"]
        except Exception as e:
            log.warning(f"  [{label}] Lecture scores partiels echouee: {e}")
    return []


def _save_partial_scores(label, scores):
    """Sauvegarde scores partiels pour reprise."""
    sf = f"{RESULTS_DIR}/{label.lower()}_partial_scores.json"
    avg = sum(scores)/len(scores) if scores else 0.0
    try:
        with open(sf, "w") as f:
            json.dump({"scores": scores, "avg": avg, "n": len(scores), "ts": time.strftime("%Y-%m-%dT%H:%M:%S")}, f, indent=2)
    except Exception as e:
        log.warning(f"  [{label}] Sauvegarde scores partiels echouee: {e}")


def manual_evaluate(module, devset, metric, label="eval"):
    """Evaluation sequentielle avec reprise et timeout 900s par step."""
    prev = _load_partial_scores(label)
    scores = list(prev)
    start_from = len(scores)
    if start_from > 0:
        log.info(f"  [{label}] Skip {start_from} steps, reprise au step {start_from+1}/{len(devset)}")
    for i, example in enumerate(devset):
        ...
        try:
            def _alarm(signum, frame):
                raise TimeoutError(f"Step {i+1} timeout apres {STEP_TIMEOUT}s")
            ...
            elapsed = time.time() - t0
            log.warning(f"  [{label}] {i+1}/{len(devset)} TIMEOUT ({elapsed:.0f}s) -> score=0.0")
            scores.append(0.0)
        except Exception as e:
            elapsed = time.time() - t0
            log.warning(f"  [{label}] {i+1}/{len(devset)} ERREUR ({elapsed:.0f}s): {e}")
            scores.append(0.0)
        save_progress(label, i+1, len(devset), score=round(sum(scores)/len(scores),3))
        _save_partial_scores(label, scores)
        time.sleep(2)
    avg = sum(scores)/len(scores) if scores else 0.0
...
def main():
    ...
    try:
        ...
        log.info("=" * 70)
        log.info("BRUCE DSPy Optimizer v3.2 (reprise + timeout) — Démarrage")
        ...
        with open(f"{RESULTS_DIR}/final_results_v31.json", "w") as f:
            json.dump(results, f, indent=2)

        save_progress("done", 0, 0, score=final_score_test, extra=results)
        log.info(f"Résultats sauvegardés dans {RESULTS_DIR}/final_results_v31.json")
        log.info("=" * 70)
